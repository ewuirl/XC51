...
C1_low_plot = plt.bar(x-0.2, C1_low, width, label='grouped low', color='#FF0000', edgecolor = "black", linewidth=2.5) # note to self: If linewidth >=3, can see a bit of bar under the axis
C1_plot = plt.bar(x, C1_means, width, label='random', color='#0000FF', edgecolor = "black", linewidth=2.5)
C1_high_plot = plt.bar(x+0.2, C1_high, width, label='grouped high', color='#FFFF00', edgecolor = "black", linewidth=2.5)

legend = plt.legend(handles=[C1_low_plot, C1_plot, C1_high_plot], loc='upper right', edgecolor='black') # 0.01 shifts the legend right slightly so it doesn't touch the y-axis tick marks
